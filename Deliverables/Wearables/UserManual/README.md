## Objetivo de la Carpeta

La carpeta `UserManual` tiene como objetivo almacenar y organizar la documentación orientada al usuario final del módulo Wearables, incluyendo guías de uso, instrucciones operativas y material de apoyo para facilitar la interacción con el sistema.

---

## Árbol de la Carpeta

```text
UserManual/
└── README.md
```

---

# 👥 Equipo de Desarrollo

| 👨‍💻 Integrante | 🔗 GitHub | 🛠️ Rol | 📝 Observaciones | Aprobado |
|----------------|-----------|---------|------------------|----------|
| **Carlos Daniel Garcia Pluma** | [@DanielGarciaPluma](https://github.com/DanielGarciaPluma) | Documentador | Sin observaciones | Aprobado ✅ |
| **Brisa Nallely Garcia Gregorio** | [@Brisgregorio](https://github.com/Brisgregorio) | Desarrollador | Sin observaciones | ✅ |

---
